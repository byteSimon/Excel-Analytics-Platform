...
const Analytics = () => {
    ...
    return (
      <div className="space-y-6 p-6">
        ...
        <div className="bg-gradient-to-r from-blue-600 via-purple-600 to-pink-600 rounded-2xl p-8 text-white relative overflow-hidden">
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h1 className="text-3xl font-bold">
                  Excel Analytics Platform Dashboard 📊
                </h1>
          ...
          <div className="absolute bottom-8 right-24 text-3xl opacity-20">
            📊
          </div>
          <div className="absolute top-1/3 right-32 text-2xl opacity-20">
            🎯
          </div>
        </div>
        ...
        <div className="text-center py-12">
          ...
          <p className="text-gray-600 dark:text-gray-300 text-lg mb-6 max-w-md mx-auto">
            Upload an Excel file to unlock powerful analytics and visualization
            tools.
          </p>
  ...
  useEffect(() => {
    if (selectedXAxis && !selectedColumns.includes(selectedXAxis))
      setSelectedXAxis("");
    if (selectedYAxis && !selectedColumns.includes(selectedYAxis))
      setSelectedYAxis("");
  }, [selectedColumns]);
  ...
  const renderChart = () => {
    ...
    if (show3D) {
      ...
      return (
        <Chart3D data={chartData} type={chart3DType} color={chartStyle.color} />
      );
    }
    ...
    switch (chartType) {
        ...
        if (selectedColumns.length < 3) {
          return (
            <div className="text-center text-xs text-gray-500">
              Select at least 3 columns for Radar Chart
            </div>
          );
        }
        ...
        return (
          <ResponsiveContainer width="100%" height={300}>
            <ReRadarChart data={radarData}>
              ...
              <PolarAngleAxis
                dataKey="axis"
                style={{ fontFamily: chartStyle.fontFamily, fontSize }}
              />
              <PolarRadiusAxis
                angle={30}
      ...
      case "bubble": {
        const sizeKey = selectedColumns.find(
          (col) =>
            col !== selectedXAxis &&
            col !== selectedYAxis &&
        ...
        if (!selectedXAxis || !selectedYAxis || !sizeKey) {
          return (
            <div className="text-center text-xs text-gray-500">
              Select X, Y, and at least one more numeric column for Bubble Chart
            </div>
          );
        }
        return (
      ...
      case "histogram": {
        const histKey =
          ...
          selectedColumns.find(
            (col) => typeof currentData[0]?.[col] === "number"
          );
        if (!histKey) {
          return (
            <div className="text-center text-xs text-gray-500">
        ...
        const values = currentData
          .map((row) => Number(row[histKey]))
          .filter((v) => !isNaN(v));
        if (values.length === 0) {
          return (
            <div className="text-center text-xs text-gray-500">
              No numeric data for Histogram
            </div>
          );
        }
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    {show3D && (
                      <Select
                        value={chart3DType}
                        onValueChange={(
                          value: "3d-bar" | "3d-scatter" | "3d-surface"
                        ) => setChart3DType(value)}
                      >
              ...
              <CardContent>
                <div
                  ...
                  style={{
                    ...
                    border: `${chartStyle.borderWidth || 1}px solid ${
                      chartStyle.borderColor || "#e5e7eb"
                    }`,
                    opacity: chartStyle.chartOpacity || 1,
                  }}
                >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <CardTitle className="text-lg flex items-center gap-2 text-white">
                      AI Data Summary
                      <Badge className="bg-yellow-400 text-black px-2 py-0.5 rounded-full text-xs font-semibold">
                        AI
                      </Badge>
                    </CardTitle>
        ...
        <DialogContent className="max-w-3xl">
          ...
          <div className="overflow-x-auto">
              ...
              <thead>
                <tr>
                  {columns.map((col) => (
                    <th
                      key={col}
                      className="px-2 py-1 border-b bg-gray-50 dark:bg-gray-800"
                    >
                      <Checkbox
                ...
                {currentData.slice(0, 10).map((row, i) => (
                  <tr key={i} className="border-t">
                    {columns.map((col) => (
                      <td
                        key={col}
                        className="px-2 py-1 text-center truncate max-w-[120px]"
                      >
                        {String(row[col])}
                      </td>
                    ))}
            ...
            <div className="text-xs text-gray-500 mt-2">
              Showing first 10 rows. Uncheck columns to exclude from chart data.
            </div>
          </div>
