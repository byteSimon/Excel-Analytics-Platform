...
const createTransporter = () => {
  if (!isEmailConfigured()) {
    console.log(
      "Email credentials not configured. Emails will be logged instead."
    );
    return null;
  }
...
<body>
    <div class="container">
        ...
        <div class="content">
            <h2>Hello ${name}!</h2>
            <p>Welcome to Excel Analytics Platform! We're excited to have you on board and can't wait to help you transform your data into powerful insights.</p>
            
            <h3>🚀 What you can do with Excel Analytics Platform:</h3>
            <ul>
        ...
        <div class="footer">
            <p>© 2024 Excel Analytics Platform Analytics. All rights reserved.</p>
            <p>This email was sent to you because you signed up for Excel Analytics Platform Analytics.</p>
        </div>
    </div>
</body>
</html>
...
<body>
    <div class="container">
        <div class="header">
            ...
            <p>Secure your Excel Analytics Platform account</p>
        </div>
        <div class="content">
            <h2>Hello ${name}!</h2>
            <p>We received a request to reset your password for your Excel Analytics Platform Analytics account.</p>
    ...
    <div class="container">
        ...
        <div class="content">
            ...
            <div style="text-align: center;">
                <a href="${
                  process.env.FRONTEND_URL || "http://localhost:5173"
                }/dashboard" class="button">View Results</a>
            </div>
...
const getPasswordChangeTemplate = (
  name: string,
  timestamp: string,
  ipAddress?: string
) => `
...
<body>
    <div class="container">
        <div class="header">
            ...
            <p>Your Excel Analytics Platform account security update</p>
        </div>
        <div class="content">
            <h2>Hello ${name}!</h2>
            <p>Your password for your Excel Analytics Platform Analytics account has been successfully changed.</p>
...
export async function sendWelcomeEmail(to: string, name: string) {
  ...
  if (!config) {
    console.log(
      `[EMAIL LOG] Welcome email would be sent to ${to} for user ${name}`
    );
    return;
  }

  const mailOptions = {
    from: config.from || "noreply@excelanalyticsplatform.com",
    to,
    subject: "🎉 Welcome to Excel Analytics Platform Analytics!",
...
export async function sendPasswordResetEmail(
  ...
  if (!config) {
    console.log(
      `[EMAIL LOG] Password reset email would be sent to ${to} with link: ${resetLink}`
    );
    return;
  }
  ...
  const mailOptions = {
    from: config.from || "noreply@excelanalyticsplatform.com",
    to,
    subject: "🔐 Reset Your Excel Analytics Platform Password",
...
export async function sendAnalysisCompleteEmail(
  ...
  if (!config) {
    console.log(
      `[EMAIL LOG] Analysis complete email would be sent to ${to} for analysis: ${analysisName}`
    );
    return;
  }
...
export async function sendPasswordChangeEmail(
  ...
  if (!config) {
    console.log(
      `[EMAIL LOG] Password change email would be sent to ${to} for user ${name}`
    );
    return;
  }
  ...
  const mailOptions = {
    from: config.from || "noreply@excelanalyticsplatform.com",
    to,
    subject: "🔐 Your Excel Analytics Platform Password Has Been Changed",
...
export async function sendTestEmail(to: string) {
  ...
  const mailOptions = {
    from: config.from || "noreply@excelanalyticsplatform.com",
    to,
    subject: "🧪 Excel Analytics Platform Email Test",
    html: `
      <h2>Email Test Successful!</h2>
      <p>This is a test email from Excel Analytics Platform Analytics to verify your email configuration is working correctly.</p>
